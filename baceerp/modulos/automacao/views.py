	...
	if nf.exists():
		...
		for m in mnf:
			...
			html += "\
						...
						<td>"+str(m.volume)+"<input type=\"hidden\" value=\""+str(m.volume)+"\" name=\"material_nota_fiscal_volume_"+str(m.id)+"\" id=\"material_nota_fiscal_volume_"+str(m.id)+"\"/></td>\
						<td>"+m.data_entrada.strftime("%d/%m/%Y")+"</td>\
						<td>"+str(m.peso)+"</td><td>"+str(m.valor)+"</td>\
						<td><input type=\"radio\" onchange=\"OrdemFabricacao.selecionaProdutoNotaFiscal(this,"+str(m.id)+")\" name=\"materia_nota_fiscal_id\" id=\"materia_nota_fiscal_"+str(m.id)+"\" value=\""+str(m.id)+"\"/></td>"
			html += "</tr>"
		html += "</tbody>"
		html += "</table>\
		</fieldset>"
...
def add_ordem_fabricacao(request):  
  ...
  return render_to_response("admin/modulos/automacao/add_ordem_fabricacao.html", {'form':form})
...
def gera_ordem_fabricacao(request):
  ...
  for x in range(0,int(request.POST["material_nota_fiscal_volume_"+request.POST["materia_nota_fiscal_id"]])):
    ...
    print int(request.POST["material_nota_fiscal_volume_"+request.POST["materia_nota_fiscal_id"]])
    of.save()
  render